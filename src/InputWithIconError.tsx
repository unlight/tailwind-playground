...
export function InputWithIconError() {
  const option3ContainerElement = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const observer = new IntersectionObserver(
      changes => {
        const [entry] = changes;
        const errorsElement =
          option3ContainerElement.current?.querySelector('.errors');
        if (entry?.isIntersecting) {
          document.body.querySelector(':scope > .errors')?.remove();
        } else {
          const node = errorsElement!.cloneNode(true);
    ...
    observer.observe(option3ContainerElement.current!);
  }, []);
    ...
    <section className="container mx-auto my-1 w-1/3 space-y-2">
      ...
      {/*<div className="float-label-container ">
        ...
        <input
          ...
          className="focus:shadow-outline float-label-control w-full border px-1 pr-8 outline-none"
        />
        <label
          htmlFor="name"
          className="float-label-self bg-white text-gray-500"
      ...
      {/* Option 3 */}
      <div className="float-label-container" ref={option3ContainerElement}>
        <div className="errors top-0 mt-1 whitespace-nowrap rounded bg-rose-600 p-2 text-xs leading-none text-white">
          <p>1. A right aligned tooltip.</p>
          <p>2. Error example text</p>
        </div>
        <input
          ...
          className="focus:shadow-outline float-label-control mb-1 w-full border border-red-400 px-1 outline-none placeholder:text-red-400"
        />
      </div>
